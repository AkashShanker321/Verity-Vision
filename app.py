...
################################################# TWITTER ###########################################################
from flask import Flask, request, jsonify, render_template, Blueprint, render_template, redirect, url_for, request, \
    flash
...
@app.route('/model_updates', methods=['GET', 'POST'])
def models_update():
    global model_path
...
def twitteroutput():
    if request.method == "POST":
        ...
        df = pd.DataFrame(
            columns=['Source', 'image_url', 'User_created', 'account_name', 'Followers', 'Friends', 'text_on_tweet',
                     'Retweet_count', 'Favorite'])

        retweet = []

        for tweet in tweepy.Cursor(api.search_tweets, q=HashValue, count=20, lang="en", since=StartDate,
                                   tweet_mode='extended', retry_count=5, retry_delay=5, include_entities=True).items(
                200):
            if 'media' in tweet.entities:
                for image in tweet.entities['media']:
                    # print(image['media_url'],tweet.user.screen_name, tweet.full_text.encode('utf-8') )
                    df = df.append(
                        {'Source': tweet.source, 'User_created': tweet.user.created_at, 'image_url': image['media_url'],
                         'account_name': tweet.user.screen_name, 'text_on_tweet': tweet.full_text.encode('utf-8'),
                         'Followers': tweet.user.followers_count, 'Friends': tweet.user.friends_count,
                         'Favorite': tweet.favorite_count, 'Retweet_count': tweet.retweet_count}, ignore_index=True)
        ...
        ################
        df = df.drop(['Source', 'Followers', 'Friends', 'text_on_tweet', 'Favorite', 'Is_Viral', 'Number_Follower',
                      'Friends_count', 'Account_created_on'], axis=1)
    ...
    if request.method == 'POST':
        ...
        if video_url != "" and util.valid_youtube_video_url(video_url):
            ...
            if file_type == "video":
                ...
                model_path = os.path.join("Trained_Models", "Fake_Face_Detection_Model_1.pth")
                label, score, fake_image_path = Fake_Face(filename, model_path)
                ...
                if label == -1:
                    ...
                    original_image = aug_3(image=np.array(original_image))['image']
                    ...
                    # Prepare the path for the original image and the steganography image
                    original_image_path = os.path.join('static', 'Test_FIM', 'original_image.jpg')
                    steg_image_path = os.path.join('static', 'Test_FIM', 'steg_image.jpg')
                    ...
                    # Prepare posts for rendering in HTML
                    posts = {
...
gelu = CustomGELU()
def Fake_Face(filename, model_path, crop=True, model_number=7):
    filename = 'fimt.' + filename.split(".")[-1]
    print("function file name: ", filename)
    d21 = 0
    print(os.path.join(UPLOAD_IMAGE_FOLDER, filename))  # static/Full images/
    final_image = os.path.join(UPLOAD_IMAGE_FOLDER, filename)
    ...
    if crop == False:
        ...
        ############Added######################
            ...
            print(image.shape)
            # For User updated model
            print("User model is selected")
            model_1 = torch.load(model_path, map_location=device)
            model_1.to(device)
            model_1.eval()
            preds = model_1(image)  # get the softmax probabilities
            pred6 = gelu(preds.squeeze())
    ...
    try:
        ...
        for i in range(0, len(boxes)):
            ...
            if model_number == 6:
                ...
                image = np.transpose(image, (2, 0, 1)).astype(np.float32)
                image = torch.tensor(image, dtype=torch.float)  # Convert to tensor
                pred = model_6(image.unsqueeze(0).to(device)).argmax()  # Pass through The Model
                ...
                print("This is User Selected Model")
            ...
            color = (0, 0, 255) if label == "Fake" else (0, 255, 0)
            print("Boxes: ", boxes)
            pt1 = (int(boxes[i][0]), int(boxes[i][1]))  # Top-left corner
            pt2 = (int(boxes[i][2]), int(boxes[i][3]))  # Bottom-right corner
            cv2.rectangle(frame_4, pt1, pt2, color, 2)  # Draw Bounding Box
            cv2.imwrite(os.path.join("test_faces_output", filename), frame_4)  # Save the Image
            # print(label + " "+ "Face Found")
            print(filename)
            print(os.path.join("test_faces_output", filename))
